...
CFeeRate GetDogecoinFeeRate(int priority)
{
    ...
    {
    case SUCH_EXPENSIVE:
        return CFeeRate(COIN / 100 * 258); // 2.58 CRC, but very carefully avoiding floating point maths
    case MANY_GENEROUS:
...
CAmount GetDogecoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetcraftcoindustFee(tx.vout, nDustLimit);
...
CAmount GetcraftcoindustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
